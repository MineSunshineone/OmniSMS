...
{
    return omnisms::translate_perl_classes(pattern);
}

esp_err_t idf_config_validate_forward_rules(const std::string& rules, std::string* message)
{
    return omnisms::validate_forward_rules(rules, message) ? ESP_OK : ESP_ERR_INVALID_ARG;
}
...
static omnisms::AppConfig to_core_config(const IdfConfig& source, bool full_export)
{
    omnisms::AppConfig out;
    out.modem.driver = "generic-3gpp";
    out.receiver = source.phoneNumber;
    out.adminPhone = source.adminPhone;
    out.numberBlacklist = source.numberBlackList;
    out.forwardRules = source.forwardRules;
    out.callNotifyEnabled = source.callNotifyEnabled;
    out.pushEnabled = source.pushEnabled;
    for (int i = 0; i < IDF_MAX_PUSH_CHANNELS; ++i) {
        const IdfPushChannel& src = source.pushChannels[i];
        omnisms::PushChannel& dst = out.pushChannels[i];
        dst.enabled = src.enabled;
        dst.type = src.type;
        dst.name = src.name;
        dst.url = full_export ? src.url : redact_secret(src.url);
        dst.key1 = full_export ? src.key1 : redact_secret(src.key1);
        dst.key2 = full_export ? src.key2 : redact_secret(src.key2);
        dst.customBody = full_export ? src.customBody : redact_secret(src.customBody);
    }
    out.email.enabled = source.emailEnabled;
    out.email.server = source.smtpServer;
    out.email.port = source.smtpPort;
    out.email.username = source.smtpUser;
    out.email.password = full_export ? source.smtpPass : redact_secret(source.smtpPass);
    out.email.recipient = source.smtpSendTo;
    out.wifi.ssid = source.wifiSsid;
    out.wifi.password = full_export ? source.wifiPass : redact_secret(source.wifiPass);
    out.web.username = source.webUser;
    out.web.password = full_export ? source.webPass : redact_secret(source.webPass);
    out.time.timezoneOffsetMin = source.tzOffsetMin;
    out.time.ntpServer = source.ntpServer;
    out.time.rebootEnabled = source.rebootEnabled;
    out.time.rebootHour = source.rebootHour;
    out.time.heartbeatEnabled = source.hbEnabled;
    out.time.heartbeatHour = source.hbHour;
    out.keepalive.enabled = source.kaEnabled;
    out.keepalive.intervalDays = source.kaIntervalDays;
    out.keepalive.action = source.kaAction;
    out.keepalive.target = source.kaTarget;
    out.keepalive.url = source.kaUrl;
    out.keepalive.profile = source.kaProfile;
    out.keepalive.lastRun = source.kaLastTime;
    out.cellular.dataEnabled = source.dataEnabled;
    out.cellular.roamingEnabled = source.roamingEnabled;
    out.cellular.apn = source.apn;
    out.cellular.operatorPlmn = source.operatorPlmn;
    out.cellular.netLedEnabled = source.netLedEnabled;
    for (int i = 0; i < IDF_MAX_SCHED_TASKS; ++i) {
        const IdfSchedTask& src = source.schedTasks[i];
        omnisms::ScheduledTaskConfig& dst = out.scheduledTasks[i];
        dst.enabled = src.enabled;
        dst.name = src.name;
        dst.profile = src.profile;
        dst.switchBack = src.switchBack;
        dst.intervalDays = src.intervalDays;
        dst.action = src.action;
        dst.target = src.target;
        dst.payload = src.payload;
        dst.lastRun = src.lastRun;
    }
    return out;
}

std::string idf_config_export_json(bool full_export)
{
    return omnisms::serialize_config_json(to_core_config(idf_config_get(), full_export));
}

esp_err_t idf_config_import_json(const std::string& json, int* applied_count, std::string* message)
{
    if (message) message->clear();
    omnisms::AppConfig imported;
    std::string parse_error;
    if (!omnisms::parse_config_json(json, imported, &parse_error)) {
        if (message) *message = parse_error;
        return ESP_ERR_INVALID_ARG;
    }
    if (imported.presence.forwardRules &&
        !omnisms::validate_forward_rules(imported.forwardRules, &parse_error)) {
        if (message) *message = parse_error;
        return ESP_ERR_INVALID_ARG;
    }
    ...
    IdfConfig next = base;
    int applied = 0;
    if (imported.presence.receiver) { next.phoneNumber = imported.receiver; ++applied; }
    if (imported.presence.adminPhone) { next.adminPhone = imported.adminPhone; ++applied; }
    if (imported.presence.numberBlacklist) { next.numberBlackList = imported.numberBlacklist; ++applied; }
    if (imported.presence.forwardRules) { next.forwardRules = imported.forwardRules; ++applied; }
    if (imported.presence.callNotifyEnabled) { next.callNotifyEnabled = imported.callNotifyEnabled; ++applied; }
    if (imported.presence.pushEnabled) { next.pushEnabled = imported.pushEnabled; ++applied; }
    if (imported.presence.pushChannels) {
        for (int i = 0; i < IDF_MAX_PUSH_CHANNELS; ++i) {
            const omnisms::PushChannel& src = imported.pushChannels[i];
            IdfPushChannel& dst = next.pushChannels[i];
            dst.enabled = src.enabled && src.type != omnisms::PUSH_TYPE_NONE;
            dst.type = src.type == omnisms::PUSH_TYPE_NONE
                           ? static_cast<uint8_t>(omnisms::PUSH_TYPE_POST_JSON) : src.type;
            dst.name = src.name;
            if (!is_redacted_secret(src.url)) dst.url = src.url;
            if (!is_redacted_secret(src.key1)) dst.key1 = src.key1;
            if (!is_redacted_secret(src.key2)) dst.key2 = src.key2;
            if (!is_redacted_secret(src.customBody)) dst.customBody = src.customBody;
            applied += 7;
        }
    }
    if (imported.presence.email) {
        next.emailEnabled = imported.email.enabled;
        next.smtpServer = imported.email.server;
        next.smtpPort = imported.email.port;
        next.smtpUser = imported.email.username;
        if (!is_redacted_secret(imported.email.password)) next.smtpPass = imported.email.password;
        next.smtpSendTo = imported.email.recipient;
        applied += 6;
    }
    if (imported.presence.wifi) {
        next.wifiSsid = imported.wifi.ssid;
        if (!is_redacted_secret(imported.wifi.password)) next.wifiPass = imported.wifi.password;
        next.wifiFromFallback = false;
        applied += 2;
    }
    if (imported.presence.web) {
        next.webUser = imported.web.username;
        if (!is_redacted_secret(imported.web.password)) next.webPass = imported.web.password;
        applied += 2;
    }
    if (imported.presence.time) {
        next.tzOffsetMin = imported.time.timezoneOffsetMin;
        next.ntpServer = imported.time.ntpServer;
        next.rebootEnabled = imported.time.rebootEnabled;
        next.rebootHour = imported.time.rebootHour;
        next.hbEnabled = imported.time.heartbeatEnabled;
        next.hbHour = imported.time.heartbeatHour;
        applied += 6;
    }
    if (imported.presence.keepalive) {
        next.kaEnabled = imported.keepalive.enabled;
        next.kaIntervalDays = imported.keepalive.intervalDays;
        next.kaAction = imported.keepalive.action;
        next.kaTarget = imported.keepalive.target;
        next.kaUrl = imported.keepalive.url;
        next.kaProfile = imported.keepalive.profile;
        next.kaLastTime = imported.keepalive.lastRun;
        applied += 7;
    }
    if (imported.presence.cellular) {
        next.dataEnabled = imported.cellular.dataEnabled;
        next.roamingEnabled = imported.cellular.roamingEnabled;
        next.apn = imported.cellular.apn;
        next.operatorPlmn = imported.cellular.operatorPlmn;
        next.netLedEnabled = imported.cellular.netLedEnabled;
        applied += 5;
    }
    if (imported.presence.scheduledTasks) {
        for (int i = 0; i < IDF_MAX_SCHED_TASKS; ++i) {
            const omnisms::ScheduledTaskConfig& src = imported.scheduledTasks[i];
            IdfSchedTask& dst = next.schedTasks[i];
            dst.enabled = src.enabled;
            dst.name = src.name;
            dst.profile = src.profile;
            dst.switchBack = src.switchBack;
    ...
    if (err != ESP_OK && message && message->empty()) *message = esp_err_to_name(err);
    return err;
}
...
esp_err_t idf_config_import_text(const std::string& text, int* applied_count)
{
    ...
    if (first != std::string::npos && text[first] == '{') {
        return idf_config_import_json(text, applied_count, nullptr);
    }
    IdfConfig base = idf_config_get();
    IdfConfig next = base;
    int applied = 0;
...
esp_err_t idf_config_save_sched_tasks(const IdfSchedTask tasks[IDF_MAX_SCHED_TASKS])
{
    ...
    for (int i = 0; i < IDF_MAX_SCHED_TASKS; ++i) {
        ...
        if (next[i].enabled && !was_enabled[i] && next[i].lastRun == 0 &&
            omnisms::epoch_valid(now)) {
            next[i].lastRun = now;
        }
    }
