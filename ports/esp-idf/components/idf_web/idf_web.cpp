...
{
    return omnisms::epoch_valid(epoch);
}
...
static esp_err_t handle_export_config(httpd_req_t* req)
{
    ...
    std::string format;
    get_query_param(req, "format", format, 16);
    bool json = format == "json";
    std::string body = json ? idf_config_export_json(full == "1")
                            : idf_config_export_text(full == "1");
    httpd_resp_set_type(req, json ? "application/json; charset=utf-8" : "text/plain; charset=utf-8");
    set_no_cache_headers(req);
    httpd_resp_set_hdr(req, "Content-Disposition",
                       json ? "attachment; filename=omnisms_config.json"
                            : "attachment; filename=sms_config.txt");
    return httpd_resp_send(req, body.c_str(), body.size());
}
...
static esp_err_t handle_import_config(httpd_req_t* req)
{
    ...
    if (!check_csrf(req)) return ESP_OK;
    std::string body;
    if (read_body(req, body, 32768) != ESP_OK) return ESP_OK;
    int applied = 0;
    std::string detail;
    size_t first = body.find_first_not_of(" \t\r\n");
    esp_err_t err = first != std::string::npos && body[first] == '{'
                        ? idf_config_import_json(body, &applied, &detail)
                        : idf_config_import_text(body, &applied);
    set_json_no_cache(req);
    if (err != ESP_OK) {
        std::string resp = "{\"success\":false,";
        json_prop(resp, "message", std::string("导入失败: ") +
                                  (detail.empty() ? esp_err_to_name(err) : detail));
        resp += "}";
...
static void keepalive_task(void* arg_raw)
{
    ...
    if (ok) {
        ...
        if (epoch_valid(now) && idf_config_set_keepalive_last(now) == ESP_OK) {
            message += "，已更新保号基准日";
        }
        idf_log_line("保号动作成功");
...
static void sched_task_worker(void* arg_raw)
{
    ...
    if (epoch_valid(now)) {
        if (ok) {
            ...
            idf_config_set_sched_last(index,
                                      omnisms::failure_retry_baseline(now, t.intervalDays));
        }
    }
    ...
    while (true) {
        ...
        if (epoch_valid(now)) {
            ...
            if (last_ka_check_ms == 0 || now_ms - last_ka_check_ms >= 3600000UL) {
                ...
                const omnisms::PeriodicState ka_state = omnisms::evaluate_periodic(
                    cfg.kaEnabled, cfg.kaLastTime, now, cfg.kaIntervalDays);
                if (ka_state == omnisms::PeriodicState::EstablishBaseline) {
                    // 首次启用/无基准日：只建立基准日，绝不立即执行——
                    // 否则刚开启保号就真发短信/USSD/跑流量，可能直接扣费
                    idf_config_set_keepalive_last(now);
                    idf_log_line("保号基准日已建立(首次启用不执行动作)");
                } else if (ka_state == omnisms::PeriodicState::Due) {
                    std::string msg;
                    bool already = false;
                    IdfKeepaliveRunView run_cfg = idf_config_get_keepalive_run_view();
                    bool still_due = omnisms::evaluate_periodic(
                        run_cfg.kaEnabled, run_cfg.kaLastTime, now,
                        run_cfg.kaIntervalDays) == omnisms::PeriodicState::Due;
                    if (still_due) {
                ...
                for (int i = 0; i < IDF_MAX_SCHED_TASKS; ++i) {
                    ...
                    const omnisms::PeriodicState task_state = omnisms::evaluate_periodic(
                        t.enabled, t.lastRun, now, t.intervalDays);
                    if (task_state == omnisms::PeriodicState::EstablishBaseline) {
                        // 老配置/时间未同步时启用的任务：先建立基准日，不立即执行
                        idf_config_set_sched_last(i, now);
                        continue;
                    }
                    if (task_state != omnisms::PeriodicState::Due) continue;
                    std::string msg;
                    bool already = false;
                    IdfSchedRunView run_cfg = idf_config_get_sched_run_view(i);
                    const IdfSchedTask& latest = run_cfg.task;
                    bool still_due = run_cfg.valid &&
                        omnisms::evaluate_periodic(latest.enabled, latest.lastRun, now,
                                                   latest.intervalDays) == omnisms::PeriodicState::Due;
                    if (still_due) {
            ...
            const omnisms::LocalDayHour local = omnisms::local_day_hour(now, cfg.tzOffsetMin);

            // 用 > 而非 !=：NTP 回拨跨过本地午夜会让 day 变小，!= 会当天重复触发
            if (omnisms::daily_due(cfg.hbEnabled, cfg.hbHour, hb_last_day,
                                   now, cfg.tzOffsetMin)) {
                hb_last_day = local.day;
                store_hb_last_day(local.day);
                IdfSmsStatus sms = idf_sms_get_status();
                char body[192];
            ...
            if (omnisms::daily_reboot_due(
                    cfg.rebootEnabled, cfg.rebootHour, rb_last_day, now, cfg.tzOffsetMin,
                    static_cast<int64_t>(uptime_ms / 1000ULL), system_idle_for_maintenance())) {
                rb_last_day = local.day;
                idf_log_line("每日定时重启...");
                vTaskDelay(pdMS_TO_TICKS(300));
...
static esp_err_t handle_keepalive(httpd_req_t* req)
{
    ...
    if (action == "reset") {
        ...
        uint32_t now = static_cast<uint32_t>(time(nullptr));
        if (!epoch_valid(now)) {
            // 时间未同步时写 0 会让周期状态判定为"建立基准"，不能冒充手动重置成功
    ...
    if (time_valid && epoch_valid(cfg.kaLastTime) && cfg.kaIntervalDays > 0) {
