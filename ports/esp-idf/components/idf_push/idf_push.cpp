...
using omnisms::json_prop;
using omnisms::trim;
using omnisms::utf8_truncate;
...
static constexpr uint8_t PUSH_RETRY_MAX = omnisms::PUSH_RETRY_MAX;
...
static_assert(IDF_MAX_PUSH_CHANNELS == omnisms::MAX_PUSH_CHANNELS,
              "ESP and core push channel counts must stay aligned");
static_assert(static_cast<int>(PUSH_TYPE_NONE) == static_cast<int>(omnisms::PUSH_TYPE_NONE) &&
              static_cast<int>(PUSH_TYPE_POST_JSON) == static_cast<int>(omnisms::PUSH_TYPE_POST_JSON) &&
              static_cast<int>(PUSH_TYPE_BARK) == static_cast<int>(omnisms::PUSH_TYPE_BARK) &&
              static_cast<int>(PUSH_TYPE_GET) == static_cast<int>(omnisms::PUSH_TYPE_GET) &&
              static_cast<int>(PUSH_TYPE_DINGTALK) == static_cast<int>(omnisms::PUSH_TYPE_DINGTALK) &&
              static_cast<int>(PUSH_TYPE_PUSHPLUS) == static_cast<int>(omnisms::PUSH_TYPE_PUSHPLUS) &&
              static_cast<int>(PUSH_TYPE_SERVERCHAN) == static_cast<int>(omnisms::PUSH_TYPE_SERVERCHAN) &&
              static_cast<int>(PUSH_TYPE_CUSTOM) == static_cast<int>(omnisms::PUSH_TYPE_CUSTOM) &&
              static_cast<int>(PUSH_TYPE_FEISHU) == static_cast<int>(omnisms::PUSH_TYPE_FEISHU) &&
              static_cast<int>(PUSH_TYPE_GOTIFY) == static_cast<int>(omnisms::PUSH_TYPE_GOTIFY) &&
              static_cast<int>(PUSH_TYPE_TELEGRAM) == static_cast<int>(omnisms::PUSH_TYPE_TELEGRAM),
              "ESP and core push type values must stay aligned");

static SemaphoreHandle_t s_mutex = nullptr;
// worker 唤醒信号：新任务入队即刻开始处理，不等 100ms 空闲轮询
static SemaphoreHandle_t s_wake_sem = nullptr;
static omnisms::FixedDueQueue<PushJob, PUSH_QUEUE_MAX> s_push_jobs;
static omnisms::FixedDueQueue<ForwardJob, FWD_QUEUE_MAX> s_forward_jobs;
static omnisms::FixedDueQueue<EmailJob, EMAIL_QUEUE_MAX> s_email_jobs;
...
static omnisms::PushChannel to_core_channel(const IdfPushChannel& ch)
{
    omnisms::PushChannel core;
    core.enabled = ch.enabled;
    core.type = ch.type;
    core.name = ch.name;
    core.url = ch.url;
    core.key1 = ch.key1;
    core.key2 = ch.key2;
    core.customBody = ch.customBody;
    return core;
}

static bool channel_valid(const IdfPushChannel& ch)
{
    return omnisms::channel_valid(to_core_channel(ch));
}

static uint32_t backoff_seconds(uint8_t attempts, uint32_t seed)
{
    return omnisms::backoff_seconds(attempts, seed);
}
                            ...
                            bool notify = false)
{
    ...
    event.sender = sender_raw ? sender_raw : "";
    event.text = text_raw ? text_raw : "";
    event.timestamp = timestamp_raw ? timestamp_raw : "";
    event.receiver = notify ? std::string() : local_phone_number();

    omnisms::HttpRequest request;
    if (!omnisms::build_push_request(to_core_channel(channel), event, notify,
                                     utc_millis(), request)) {
        return false;
    }
    ...
    int code = 0;
    esp_err_t err = http_request(request.url, request.method.c_str(),
                                 request.content_type.c_str(), request.body, code);
                                    ...
                                    uint32_t completion_id = 0)
{
    ...
    int64_t due = esp_timer_get_time() + static_cast<int64_t>(delay_sec) * 1000000LL;
    // 旧任务可能已经对应收件箱里的"已排队"短信，不能为新任务静默踢掉。
    return s_push_jobs.enqueue(std::move(job), due);
}

static int push_queue_free_locked()
{
    return static_cast<int>(s_push_jobs.free());
}
                                     ...
                                     uint32_t inbox_id = 0, uint32_t completion_id = 0)
{
    ...
    job.completionId = completion_id;
    int64_t due = esp_timer_get_time() + static_cast<int64_t>(delay_sec) * 1000000LL;
    return s_email_jobs.enqueue(std::move(job), due);
}

static int email_queue_depth_locked()
{
    return static_cast<int>(s_email_jobs.size());
}

static bool enqueue_forward_job_locked(const ForwardJob& src, uint32_t delay_sec)
{
    int64_t due = esp_timer_get_time() + static_cast<int64_t>(delay_sec) * 1000000LL;
    return s_forward_jobs.enqueue(src, due);
}
...
static bool pop_forward_job(ForwardJob& out)
{
    ...
    bool found = s_forward_jobs.take_ready(esp_timer_get_time(), out);
    xSemaphoreGive(s_mutex);
    return found;
}
...
static bool process_forward_one()
{
    ...
    omnisms::ForwardDecision fd =
        omnisms::eval_forward_rules(cfg.forwardRules, job.sender, job.text);
    ...
    if (s_mutex && xSemaphoreTake(s_mutex, portMAX_DELAY) == pdTRUE) {
        ...
        if (!enqueue_failed && will_queue_email) {
            const omnisms::EmailContent content = omnisms::build_sms_email(
                omnisms::SmsEvent{job.sender, job.text, job.timestamp, receiver});
            email_queued = enqueue_email_job_locked(
                content.subject, content.body, 0, 0, job.inboxId, completion_id);
...
static bool process_push_one()
{
    ...
    bool picked = s_push_jobs.take_ready_if(now, job, [now](const PushJob& pending) {
        return !channel_cooling(pending.channel, now);  // 连续失败的通道冷却期内跳过
    });
    xSemaphoreGive(s_mutex);
    if (!picked) {
        s_busy.store(false, std::memory_order_relaxed);
...
static bool process_email_one()
{
    ...
    if (!cfg.emailEnabled) {
        ...
        if (s_mutex && xSemaphoreTake(s_mutex, portMAX_DELAY) == pdTRUE) {
            bool purged = false;
            s_email_jobs.for_each([&](EmailJob& j) {
                purged = true;
    ...
    bool picked = s_email_jobs.take_ready(now, job);
    xSemaphoreGive(s_mutex);
    if (!picked) {
        s_busy.store(false, std::memory_order_relaxed);
        return false;
...
int idf_push_forward_queue_depth(void)
{
    ...
    int n = static_cast<int>(s_forward_jobs.size());
    xSemaphoreGive(s_mutex);
    return n;
}
...
int idf_push_retry_queue_depth(void)
{
    if (!ensure_init()) return 0;
    if (!s_mutex || xSemaphoreTake(s_mutex, pdMS_TO_TICKS(100)) != pdTRUE) return 0;
    int n = static_cast<int>(s_push_jobs.size());
    xSemaphoreGive(s_mutex);
    return n;
}
