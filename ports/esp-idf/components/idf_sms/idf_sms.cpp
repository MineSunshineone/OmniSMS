...
using omnisms::is_valid_phone_number;
using omnisms::number_blacklisted;
...
static omnisms::PduCodec s_pdu(4096);
...
static bool process_admin_command(const std::string& sender, const std::string& text,
                                  const IdfSmsProcessView& cfg)
{
    const omnisms::AdminCommandDecision decision = omnisms::evaluate_admin_command(
        cfg.adminPhone, sender, text, esp_timer_get_time() / 1000000LL,
        s_admin_sms_busy.load(std::memory_order_relaxed),
        s_admin_reset_pending.load(std::memory_order_relaxed));
    if (!decision.handled()) return false;

    idf_logf("处理管理员命令 from=%s", sender.c_str());
    if (decision.status == omnisms::AdminCommandStatus::SmsFormatError ||
        decision.status == omnisms::AdminCommandStatus::InvalidTarget ||
        decision.status == omnisms::AdminCommandStatus::InvalidContent ||
        decision.status == omnisms::AdminCommandStatus::SmsBusy) {
        const char* message = omnisms::admin_command_message(decision.status);
        idf_log_line(message);
        idf_push_enqueue_email("命令执行失败", message);
        return true;
    }

    if (decision.status == omnisms::AdminCommandStatus::SendSms) {
        idf_logf("管理员命令目标号码: %s", decision.target.c_str());
        bool expected = false;
        if (!s_admin_sms_busy.compare_exchange_strong(expected, true, std::memory_order_acq_rel)) {
            idf_log_line("已有管理员短信任务在执行，拒绝新的 SMS 命令");
            idf_push_enqueue_email("命令执行失败",
                                   omnisms::admin_command_message(omnisms::AdminCommandStatus::SmsBusy));
            return true;
        }
        ...
        arg->target = decision.target;
        arg->content = decision.content;
        arg->command = decision.command;
    ...
    if (decision.status == omnisms::AdminCommandStatus::ResetTooSoon ||
        decision.status == omnisms::AdminCommandStatus::ResetPending) {
        const char* message = omnisms::admin_command_message(decision.status);
        idf_log_line(message);
        idf_push_enqueue_email("RESET已忽略", message);
        return true;
    }
    bool expected = false;
...
static void emit_concat_sms(const std::string& sender, const std::string& text,
                            const std::string& timestamp)
{
    process_sms_content(sender.c_str(), text.c_str(), timestamp.c_str());
}

static void observe_concat(const omnisms::ConcatAssembler::Notice& notice)
{
    using NoticeKind = omnisms::ConcatAssembler::NoticeKind;
    switch (notice.kind) {
        case NoticeKind::PartReceived:
            idf_logf("收到长短信分段 ref=%d %d/%d",
                     notice.reference, notice.part, notice.total);
            break;
        case NoticeKind::InvalidPart:
            idf_logf("长短信分段参数超限 part=%d total=%d，按单条处理",
                     notice.part, notice.total);
            break;
        case NoticeKind::SlotEvicted:
            idf_logf("长短信槽位耗尽，先合并已收 %d/%d 段再复用槽位",
                     notice.received, notice.total);
            break;
        case NoticeKind::TimedOut:
            idf_logf("长短信等待超时，已合并现有 %d/%d 段",
                     notice.received, notice.total);
            break;
    }
}

static omnisms::ConcatAssembler s_concat_assembler(emit_concat_sms, observe_concat);
...
static void process_sms_content(const char* sender_raw, const char* text_raw, const char* timestamp_raw)
{
    ...
    if (omnisms::admin_sender_matches(cfg.adminPhone, sender)) {
        idf_log_line("收到管理员短信，检查命令...");
    }
    if (process_admin_command(sender, text, cfg)) {
...
static void handle_decoded_pdu(const omnisms::DecodedSms& sms)
{
    s_concat_assembler.feed(sms, esp_timer_get_time());
}
...
static bool decode_pdu_line(const std::string& line)
{
    ...
    omnisms::DecodedSms decoded;
    if (!s_pdu.decode(line, decoded)) {
...
{
    s_concat_assembler.expire(esp_timer_get_time());
}
...
esp_err_t idf_sms_send_text(const std::string& phone_raw, const std::string& text_raw, std::string& message)
{
    ...
    // 16 位参考号从熵源取初值后自增，core 只在长短信时写入 UDH。
    static std::atomic<uint16_t> s_csms_ref{static_cast<uint16_t>(esp_timer_get_time() & 0x7FFF)};
    uint16_t csms = s_csms_ref.fetch_add(1, std::memory_order_relaxed);
    ...
    bool encoded = s_pdu.encode_text(phone, text, csms, parts, &message);
    xSemaphoreGive(s_pdu_mutex);
    if (!encoded) {
        if (message.empty()) message = "PDU 编码失败";
        idf_sent_add(phone.c_str(), text.c_str(), false);
        return message == "号码或内容无效" ? ESP_ERR_INVALID_ARG : ESP_FAIL;
    }

    esp_err_t err = ESP_OK;
    for (size_t i = 0; i < parts.size(); ++i) {
        char cmd[32];
        snprintf(cmd, sizeof(cmd), "AT+CMGS=%d", parts[i].tpduLength);
        std::string resp;
        err = idf_modem_send_pdu(cmd, parts[i].payload.c_str(), 20000, resp);
        if (err != ESP_OK) {
            if (parts.size() > 1) {
                char buf[96];
